...
def create_features(input_csv, output_csv):
    ...
    # Sort chronologically by asset and date
    ...
    # Process per asset to avoid cross-asset lookback leakage
    asset_dfs = []

    for asset, asset_df in df.groupby('asset'):
        asset_df = asset_df.copy()

        # A. Candle structure (Day t only)
        ...
        # Range expansion vs prior day (Use shift(1) to avoid lookahead)
        ...
        # B. Multi-day context (shifted/rolling on past data ONLY)
        ...
        for w in [2, 3, 5, 10, 20]:
            ...
            # Volatility regime (ATR-like proxy)
        ...
        # Cumulative sum with reset (consecutive days)
        # Since this can be slow, let's use a rolling sum as a proxy for simplicity or implement a quick reset
    ...
    # Merge BTC features back on date
    combined_df = combined_df.merge(btc_df, on='date', how='left')

    # F. Calendar features (Based on date only, no future info)
    ...
    # Clean up and handle NaNs (from rolling windows)
    # We should drop rows with NaNs to ensure clean training, but first let's see how many
    print(f"Total rows before NaN drop: {len(combined_df)}")

    # Select feature columns + target + metadata
    ...
    print(f"Total rows after NaN drop: {len(final_features_df)}")
